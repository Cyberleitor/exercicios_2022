# This bot is a version of the "Uncle Pavê" and was made to a friend that is a streamer.
...
class Bot(commands.Bot):
	def __init__(self):
		super().__init__(token=bot_token, prefix='!', initial_channels=['summers_jokes', 'john_nothing', 'summereletrohitstv'])
	async def event_ready(self):
		print(f'Logged in as | {self.nick}')
	async def event_message(self, ctx):
		if ctx.echo:
			return
		if(ctx.content.lower() == '!piada'):
			quote = jokes_list
			await ctx.channel.send(random.choice(quote))
bot = Bot()
bot.run()
